  ...
  erpnext.PointOfSale.ItemCart = class {
    ...
    bind_events() {
      ...
      this.$component.on("click", ".checkout-btn", async function() {
        if ($(this).attr("style").indexOf("--btn-primary") == -1)
          return;
...
Return`,
    ...
    get_invoice_html(invoice) {
      ...
      return `<div class="invoice-wrapper" data-invoice-doctype="${invoice.doctype}" data-invoice-name="${escape(invoice.name)}">
				<div class="invoice-name-date">
					<div class="invoice-name">${invoice.name}</div>
...
`);
      ...
      this.recent_order_list = new erpnext.PointOfSale.PastOrderList({
        ...
        events: {
          open_invoice_data: (doctype, name) => {
            if (!["POS Invoice", "Sales Invoice"].includes(doctype))
              return;
    ...
    make_invoice_frm(doctype) {
      return new Promise((resolve) => {
        if (this.frm && this.frm.doctype == doctype) {
          ...
          frappe.model.with_doctype(doctype, () => {
            this.frm = this.get_new_frm(void 0, doctype);
            this.frm.doc.items = [];
            this.frm.doc.is_pos = 1;
            if (doctype == "Sales Invoice")
              this.frm.doc.is_created_using_pos = 1;
    ...
    async make_return_invoice(doc) {
      return frappe.call({
        method: doc.doctype == "POS Invoice" ? "erpnext.accounts.doctype.pos_invoice.pos_invoice.make_sales_return" : "erpnext.accounts.doctype.sales_invoice.sales_invoice.make_sales_return",
        args: {
...
//# sourceMappingURL=point-of-sale.bundle.HYZNMMVT.js.map
