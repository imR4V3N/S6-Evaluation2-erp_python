(() => {
  ...
  $.extend(erpnext, {
    ...
    toggle_naming_series: function() {
      if (cur_frm && cur_frm.fields_dict.naming_series && cur_frm.meta.naming_rule == 'By "Naming Series" field') {
  ...
  $.extend(erpnext.utils, {
    ...
    get_fiscal_year: function(date, with_dates = false, raise_on_missing = true) {
      ...
      let fiscal_year = "";
      if (frappe.boot.current_fiscal_year && date >= frappe.boot.current_fiscal_year[1] && date <= frappe.boot.current_fiscal_year[2]) {
        if (with_dates)
          fiscal_year = frappe.boot.current_fiscal_year;
        else
          fiscal_year = frappe.boot.current_fiscal_year[0];
      } else {
        frappe.call({
  ...
  $.extend(erpnext.stock_reservation, {
    ...
    get_parms(frm, table_name) {
      ...
      params["method"] = {
        "Sales Order": "delivered_qty",
        "Work Order": "erpnext.manufacturing.doctype.work_order.work_order.make_stock_reservation_entries"
      }[frm.doc.doctype];
      return params;
    },
    ...
    render_items(frm, parms) {
      ...
      frm.doc[parms.table_name].forEach((item) => {
        if (frm.doc.reserve_stock) {
          let unreserved_qty = (flt(item[qty_field]) - (item.stock_reserved_qty ? flt(item.stock_reserved_qty) : flt(item[dispatch_qty_field]) * flt(item.conversion_factor || 1))) / flt(item.conversion_factor || 1);
          if (unreserved_qty > 0) {
            let args = {
              __checked: 1,
    ...
    cancel_stock_reservation(dialog, frm) {
      ...
      if (((_a = data.sr_entries) == null ? void 0 : _a.length) > 0) {
        frappe.call({
  ...
  // frappe-html:/home/raven/Workspace/Evaluation2/Evaluation2-erp_python/erp-local/apps/erpnext/erpnext/public/js/templates/visual_plant_floor_template.html
...
{% }); %}`;
  ...
  var VisualPlantFloor = class {
    ...
    update_status(data) {
      ...
      if (data.status_image) {
        workstation_card$.find(".workstation-image-container").append(`<img class="workstation-image-cls" src="${data.status_image}" />`);
      } else {
        workstation_card$.find(".workstation-image-container").append(
      ...
      $.each(["company", "customer"], function(i, fieldname) {
        if (frappe.meta.has_field(me.frm.doc.doctype, fieldname) && !["Purchase Order", "Purchase Invoice"].includes(me.frm.doc.doctype)) {
          if (!me.frm.doc[fieldname]) {
            frappe.msgprint(__("Please specify") + ": " + __(frappe.meta.get_label(me.frm.doc.doctype, fieldname, me.frm.doc.name)) + ". " + __("It is needed to fetch Item Details."));
            valid = false;
          }
        }
      });
  ...
  // frappe-html:/home/raven/Workspace/Evaluation2/Evaluation2-erp_python/erp-local/apps/erpnext/erpnext/public/js/templates/item_selector.html
...
`;
  ...
  // frappe-html:/home/raven/Workspace/Evaluation2/Evaluation2-erp_python/erp-local/apps/erpnext/erpnext/public/js/templates/item_quick_entry.html
...
`;
  ...
  // frappe-html:/home/raven/Workspace/Evaluation2/Evaluation2-erp_python/erp-local/apps/erpnext/erpnext/public/js/templates/call_link.html
...
`;
  ...
  // frappe-html:/home/raven/Workspace/Evaluation2/Evaluation2-erp_python/erp-local/apps/erpnext/erpnext/public/js/templates/crm_activities.html
...
.form-footer {
  ...
  // frappe-html:/home/raven/Workspace/Evaluation2/Evaluation2-erp_python/erp-local/apps/erpnext/erpnext/public/js/templates/crm_notes.html
        ...
        setup_queries() {
          ...
          if (this.frm.fields_dict["items"].grid.get_field("item_code")) {
            this.frm.set_query("item_code", "items", function() {
              ...
              return {
                query: "erpnext.controllers.queries.item_query",
                filters: { is_sales_item: 1, customer, has_variants: 0 }
              };
            });
          }
...
//# sourceMappingURL=erpnext.bundle.T46QB754.js.map
