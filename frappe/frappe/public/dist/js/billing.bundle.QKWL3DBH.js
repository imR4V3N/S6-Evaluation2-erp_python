(()=>{var i="https://frappecloud.com",r=!1;$(document).ready(function(){frappe.boot.is_fc_site&&frappe.boot.setup_complete===1&&!frappe.is_mobile()&&frappe.user.has_role("System Manager")&&frappe.call({method:"frappe.integrations.frappe_providers.frappecloud_billing.current_site_info",callback:t=>{if(!(t!=null&&t.message))return;let e=t.message,a=new Date(e.trial_end_date);i=e.base_url,r=e.is_fc_user,e.trial_end_date&&a>new Date&&$(".layout-main-section").before(p(e.trial_end_date)),d(),$(".login-to-fc, .upgrade-plan-button").on("click",function(){s()})}})});function d(){$(".dropdown-navbar-user .dropdown-menu .dropdown-divider").before('<div class="dropdown-item login-to-fc" target="_blank">Manage Billing</div>')}function s(){window.open(`${i}/dashboard/sites/${frappe.boot.sitename}`,"_blank")}function p(t){let e=new Date(t),a=new Date,o=e-a,n=Math.ceil(o/(1e3*60*60*24)),l=n>1?`${n} days`:`${n} day`;return $(`
			<style>
				.trial-banner {
					display: flex;
...
`)}})();
//# sourceMappingURL=billing.bundle.QKWL3DBH.js.map
